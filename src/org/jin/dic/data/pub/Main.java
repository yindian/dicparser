...
public class Main {
  ...
  public static void main(String[] args) throws IOException, BadFormatException{
    ...
    if(args[0].equalsIgnoreCase("-d")) engine = "org.jin.dic.data.ks.dic.KSDictDataEngine";
    else if(args[0].equalsIgnoreCase("-g")){
      engine = "org.jin.dic.data.ks.dic.CSDictDataEngine";
      File dicFile = new File(desFld);
      srcFld = getFld(srcFld) + dicFile.getName();
      desFld = dicFile.getParent();
    }else if(args[0].equalsIgnoreCase("-e")){
      engine = "org.jin.dic.data.ks.dic.KSDictDataEngine";
      desFld = "@" + desFld;
    }else return;
