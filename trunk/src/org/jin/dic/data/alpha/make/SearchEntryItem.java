...
class SearchEntryItem{
  ...
  private SearchEntryItem getLeaf(){// left or right most single leaf
    if(childrenItemMap.size() == 0) return this;
    else return ((SearchEntryItem) childrenItemMap.values().iterator().next()).getLeaf();
  }

  private SearchEntryItem getItem(char c){// if none in map, put a new one
    Character cr = new Character(Character.toUpperCase(c));
    SearchEntryItem child = (SearchEntryItem) childrenItemMap.get(cr);
    if(child == null){
      child = new SearchEntryItem();
      child.setChar(c);
  ...
  int traverse(SearchEntryItem item, int type, List itemsList){
    int result = 0;
    Iterator i = item.childrenItemMap.values().iterator();
    SearchEntryItem child;
    boolean firstChild = true;
    int childrenNum = item.childrenItemMap.size();
    int pathNum;
    while(i.hasNext()){
      child = (SearchEntryItem) i.next();
      switch(type){
  ...
  void print(SearchEntryItem child){
    Logger.info_("[");
  ...
  public static void main(String[] args){
    SearchEntryItem root = new SearchEntryItem();
    root.minwords = 1;
    root.setChar((char) 0);
    root.addBranchPath("!Kung", 0x500);
